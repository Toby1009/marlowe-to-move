...
module test::fallback_case_auto_marlowe {
    ...
    /// @dev Stage 0 / Case 0: Role(Buyer) 存款
    public fun deposit_stage_0_case_0(
        ...
        assert!(coin::value(&deposit_coin) == internal_eval(contract, vector[2, 0, 0, 0, 0, 0, 0, 1, 244], ctx), E_WRONG_AMOUNT);
        assert!(tx_context::epoch_timestamp_ms(ctx) < 1777679999000, E_TIMEOUT_PASSED);
        assert_role(contract, role_nft, string::utf8(b"Buyer"));

        // 2. 執行存款
        internal_deposit<sui::sui::SUI>(contract, string::utf8(b"Role(Buyer)"), deposit_coin, ctx);
        // 3. 推進狀態機
        
        // 自動呼叫鏈：執行下一個自動 stage
        contract.stage = 1;
        internal_pay_stage_1(contract, ctx);

    }

    /// @dev Stage 0: 處理超時 (Timeout: 1777679999000)
    public fun timeout_stage_0(
        ...
        assert!(current_time >= 1777679999000, E_TIMEOUT_NOT_YET);
    ...
    /// @dev Stage 1: 自動支付 (from Role(Buyer) to Party(Role(Seller)))
    fun internal_pay_stage_1(
        contract: &mut Contract,
        ctx: &mut TxContext
    ) {
        // 1. 驗證
        ...
        // 2. 求值/查找收款人
        let amount = internal_eval(contract, vector[2, 0, 0, 0, 0, 0, 0, 1, 244], ctx);
        let from_party_id = string::utf8(b"Role(Buyer)");
        
        // 從註冊表查找 Role 的地址
        assert!(table::contains(&contract.role_registry, string::utf8(b"Seller")), E_ROLE_NOT_FOUND);
        let receiver_addr = *table::borrow(&contract.role_registry, string::utf8(b"Seller"));
