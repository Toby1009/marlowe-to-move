...
module test::simple_voting_marlowe {
    ...
    /// @dev Stage 0 / Case 0: Choice vote_result by Role(Voter)
    public fun choice_stage_0_case_0(
        ...
        assert!(tx_context::epoch_timestamp_ms(ctx) < 1775174399000, E_TIMEOUT_PASSED);
        assert_role(contract, role_nft, string::utf8(b"Voter"));
        assert!((chosen_num >= 0 && chosen_num <= 1), E_INVALID_CHOICE);

        // 2. 記錄 Choice
        
        assert!(!table::contains(&contract.choices, string::utf8(b"vote_result:Role(Voter)")), E_CHOICE_ALREADY_MADE);
        table::add(&mut contract.choices, string::utf8(b"vote_result:Role(Voter)"), chosen_num);
        ...
        // 自動呼叫鏈：執行下一個自動 stage
        contract.stage = 1;
        internal_if_stage_1(contract, ctx);

    }

    /// @dev Stage 0: 處理超時 (Timeout: 1775174399000)
    public fun timeout_stage_0(
        ...
        assert!(current_time >= 1775174399000, E_TIMEOUT_NOT_YET);
    ...
    fun internal_if_stage_1(
        ...
        // 1. 求值 Observation
        let condition_bytecode = vector[11, 23, 118, 111, 116, 101, 95, 114, 101, 115, 117, 108, 116, 58, 82, 111, 108, 101, 40, 86, 111, 116, 101, 114, 41, 2, 0, 0, 0, 0, 0, 0, 0, 1, 31, 2, 0, 0, 0, 0, 0, 0, 0, 1, 11, 23, 118, 111, 116, 101, 95, 114, 101, 115, 117, 108, 116, 58, 82, 111, 108, 101, 40, 86, 111, 116, 101, 114, 41, 31, 40];
