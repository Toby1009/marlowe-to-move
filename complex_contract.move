...
module test::generated_marlowe {
    ...
    const E_STACK_UNDERFLOW: u64 = 11; // New Error
    ...
    fun internal_pay(contract: &mut Contract, src: String, recipient: address, amt: u64, ctx: &mut TxContext) {
        ...
        // Partial Payment Logic (Marlowe Semantics)
        // 1. Check if account exists
        if (!table::contains(&contract.accounts, src)) {
            return // No funds, pay 0
        };
        ...
        // 3. Calculate actual payment amount (Min(requested, available))
        let pay_amt = if (available >= amt) { amt } else { available };

        if (pay_amt > 0) {
            // Deduct from internal account
            *b = available - pay_amt;

            // Deduct from Vault and Transfer
            let vault = table::borrow_mut(&mut contract.vaults, token);
            // Safety check: Vault should always have enough if Account has enough
    ...
    fun internal_eval(contract: &Contract, bytecode: vector<u8>, ctx: &TxContext): u64 {
        ...
        while (i < len) {
            ...
            if (op == OP_ZW) {
                ...
                i = i + 1;
                let mut party_bytes = vector::empty<u8>();
                let mut k = 0;
                ...
                i = i + 1;
                let mut token_bytes = vector::empty<u8>();
                k = 0;
                ...
                i = i + 1;
                let mut choice_bytes = vector::empty<u8>();
                let mut k = 0;
                ...
                i = i + 1;
                let mut use_bytes = vector::empty<u8>();
                let mut k = 0;
    ...
    public fun choice_stage_0_case_0(
        ...
        // 1. 驗證
        assert!(contract.stage == 0, E_WRONG_STAGE);        assert_role(contract, role_nft, string::utf8(b"Oracle"));        assert!((chosen_num >= 1 && chosen_num <= 10), E_INVALID_CHOICE);
    ...
    public fun notify_stage_0_case_1(
        ...
        // 1. 驗證
        assert!(contract.stage == 0, E_WRONG_STAGE);        assert!(internal_eval(contract, vector[2, 0, 0, 0, 0, 0, 0, 0, 1], ctx) == 1, E_ASSERT_FAILED);
